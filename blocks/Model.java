...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        if (piece == null) {
            ...
            for (int pRow = 0; pRow < piece.height(); pRow += 1) {
                for (int pCol = 0; pCol < piece.width(); pCol += 1) {
                    if (get(pRow + row, pCol + col) && piece.get(pRow, pCol)) {
                        return false;
                    }
                }
            }
    ...
    void clearFilledLines() {
        ...
        for (int i = 0; i < _height; i += 1) {
            if (counts[0][i] == _width) {
                nrows += 1;
                bool += 1;
                for (int iterCol = 0; iterCol < _width; iterCol += 1) {
                    _cells[i][iterCol] = false;
                }
            }
        }
        for (int j = 0; j < _width; j += 1) {
            if (counts[1][j] == _height) {
                ncols += 1;
                bool += 1;
                for (int iterRow = 0; iterRow < _height; iterRow += 1) {
                    _cells[iterRow][j] = false;
                }
            }
        }
        if (bool > 0) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int overlapped = nrows * ncols;
        int numCellsCleared = (nrows * _width) + (ncols * _height);
        int bonus = _streakLength * numCellsCleared;
        _score = (numCellsCleared - overlapped) + bonus;
        return _score;
    }
    ...
    boolean roundOver() {
        for (int i = 0; i < _hand.size(); i += 1) {
            if (placeable(_hand.get(i)) && _hand.get(i) != null) {
                return false;
            }
        }
        return true;
